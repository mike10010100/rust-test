...
async fn test_store_list_all() {
    ...
    store
        .insert(JobMetadata::new(id1, JobSchedule::Immediate))
        .await
        .unwrap();
    store
        .insert(JobMetadata::new(
            id2,
...
async fn test_store_runnable_jobs_filtering() {
    ...
    store
        .insert(JobMetadata::new(immediate_id, JobSchedule::Immediate))
        .await
        .unwrap();
    ...
    let mut past_job = JobMetadata::new(
        delayed_past_id,
        JobSchedule::Delayed(Duration::from_secs(5)),
    );
...
async fn test_store_job_lifecycle_transitions() {
    ...
    // Complete run (Failed)
    let failed = store
        .complete_run(id, Err("Something went wrong".to_string()))
        .await
        .unwrap();
    assert_eq!(
        failed.status,
        JobStatus::Failed("Something went wrong".to_string())
    );
}
...
async fn test_store_periodic_rescheduling() {
    ...
    store
        .insert(JobMetadata::new(id, JobSchedule::Interval(interval)))
        .await
        .unwrap();
...
async fn test_store_concurrent_access() {
    ...
    for _i in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            let id = JobId::new();
            store_clone
                .insert(JobMetadata::new(id, JobSchedule::Immediate))
                .await
                .unwrap();
